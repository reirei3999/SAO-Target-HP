...
1. [Releases](../../releases) に公開版がある場合は、`sao-target-hp-1.2.9.jar` をダウンロードします。公開版がない場合は、[Actions](../../actions) の最新ビルド成果物から同名のJARを取得します。
...
完成したMODは `build/libs/sao-target-hp-1.2.9.jar` に出力されます。`-dev.jar` は開発用の中間成果物なので、導入には使用しないでください。
