...
- ターゲット上部の赤いダイヤ型ピンと、その左側に追従する斜め端のHPゲージ
- バニラのボスバーを独自デザインで表示

## 導入方法

1. [Releases](../../releases) に公開版がある場合は、`sao-target-hp-1.2.4.jar` をダウンロードします。公開版がない場合は、[Actions](../../actions) の最新ビルド成果物から同名のJARを取得します。
...
完成したMODは `build/libs/sao-target-hp-1.2.4.jar` に出力されます。`-dev.jar` は開発用の中間成果物なので、導入には使用しないでください。
